...
type
  ...
  TForm1 = class(TForm)
    ...
    procedure TimerInMenuTeleportTimer(Sender: TObject);
...
begin
  ...
  LabelAirbrakeTime.Caption :=
    'Airbrake time: ' + IntToStr(AirbrakeTime div 60) + ':' +
    IntToStr(AirbrakeTime mod 60);
...
begin
  ...
  TimerInMenuTeleport.Enabled := True;
  ...
  WriteFloat(-99, LocPlayer.dwAddPosZ);
  Inc(Telecount);
  ConfigFile.WriteInteger('Telecount', 'count', Telecount);
  }
end;
...
procedure TForm1.TimerInMenuTeleportTimer(Sender: TObject);
begin
  if (not LocPlayer.bInMenu) then
  begin
    WriteFloat(InMenuX, LocPlayer.dwAddPosX);
    WriteFloat(InMenuY, LocPlayer.dwAddPosY);
    WriteFloat(-99, LocPlayer.dwAddPosZ);
    Inc(Telecount);
    ConfigFile.WriteInteger('Telecount', 'count', Telecount);
    TimerInMenuTeleport.Enabled := False;
  end;
end;
...
begin
  LocPlayer.bInMenu := boolean(ReadDword($400000 + $76B964));
